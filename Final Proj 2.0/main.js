...
$('table tr td').click(function(){
    if( $(this).text() == "" && play){
        ...
        if (findWinner() != -1 && findWinner()!= ""){
            if(findWinner()=="X"){
                $('body').append('<div class="winner"><span>Winner</span> Player X</div>');
                $('body').append('<button onclick="location.reload()">Play Again</button>');
                $('.winner').css('background-color', '#99e0de');
                $('.button').css('color', '#f03e5f');
            } else {
                $('body').append('<div class="winner "><span>Winner</span> Player O</div>');
                $('body').append('<button onclick="location.reload()">Play Again</button>');
